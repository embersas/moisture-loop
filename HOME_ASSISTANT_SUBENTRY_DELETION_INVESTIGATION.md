...
| Can it OFF safely afterward? | Yes, if it retained immutable actuator configuration and its lifetime is not tied to the removed MoistureLoop entities/device. |
...
- `async_update_reload_and_abort` updates first, then raises `ValueError` if an update listener exists. It therefore cannot remain once MoistureLoop adds a listener: [`config_entries.py:3446-3479`](https://github.com/home-assistant/core/blob/2025.9.0/homeassistant/config_entries.py#L3446-L3479).
...
Provide a MoistureLoop-specific "Delete Zone" workflow/action.
...
8. **What if another update occurs while preparation runs?**
   ...
   Core creates another listener task. MoistureLoop must use:
...
4. **Exact deletion sequence**
   ...
   Native UI
     ...
     -> Core clears MoistureLoop registry device/entities
