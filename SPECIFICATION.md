# MoistureLoop — v0.1 Technical Specification
...
> **Revision note (behavioural, 2026-08-27):** spec.6 is a normative behavioural correction to the initial operational state of a newly configured zone. First live first-user setup showed that a fresh zone started `enabled=true` in IDLE, so completing the Add zone flow while the real sensor already read below the start threshold — with a healthy actuator and every safety gate passing — began AUTO irrigation immediately. That was consistent with the controller but is not an acceptable default for a first release that commands physical water. A newly configured zone therefore now begins `zone_runtime.enabled = false` in state DISABLED, and no AUTO or MANUAL admission is possible for it until the user explicitly enables it through a supported control path (§8, §9, §24.4 **Add**, I20, LC14). This applies to fresh zone creation only: every existing zone keeps its persisted `enabled` value across restart, reload, reconfiguration, same-actuator delete/re-add, schema migration, and A -> B replacement, so the new default is never a migration that disables existing users. The five controller states, T1-T59 (IDs, topology, guards, actions, destinations, reasons), I1-I37, Store schema 2, the strict §23.4 verification contract, the actuator-hazard model, accounting, and the Home Assistant 2025.9.0 minimum are all unchanged; creation derives DISABLED through the existing §24.4 `enabled`-first derivation rather than any new transition row. The normative behavioural inventory gains exactly one new ID, LC14, for the previously unowned fresh-zone default, so §39.2 becomes **135** IDs (LC1-LC14) instead of 134. All settled spec.5 behaviour remains normative except where this correction explicitly strengthens it.
>
...
>
> **Revision note (spec.8, pulse ceiling and manual buttons, 2026-09-03):** spec.8 is a normative revision to the §9 duration envelope and the §28.3 control surface, requested by the first live operator after a season of real use. Two independent changes:
>
> 1. **`pulse_duration` ceiling 30 min -> 60 min**, with the §9 defaults restated around a 60-minute default pulse: `max_cycles` 4 -> 2, `max_session_runtime` 30 min -> 2 h 5 min, `max_daily_runtime` 60 min -> 4 h, `manual_max_duration` 30 min -> 60 min. The new session default carries five minutes of deliberate headroom above two nominal pulses because `G-SESS` fits the *next whole pulse* against accrued runtime, and a pulse accrues slightly more than its nominal duration through actuator confirm latency; an exact 2 h would therefore refuse the second cycle and silently strand the `max_cycles` default. No bound is *relaxed* below an existing floor, no cap is raised beyond its existing maximum, and the dynamic `pulse duration <= session <= 4 h` / `pulse duration <= daily <= 12 h` relations are unchanged, so every previously valid configuration remains valid and no persisted zone is migrated or reinterpreted.
>
> 2. **Three fixed-duration manual watering buttons** (`manual_pulse_15`, `manual_pulse_30`, `manual_pulse_60`) are added to §28.3, replacing the previous blanket prohibition on a manual-start button. The prohibition's rationale is preserved and made precise: what a button cannot safely supply is an open-ended or operator-typed duration, not a bounded one fixed in the control's own identity. Each button is presentation over the existing `async_manual_start` path and adds no admission authority: every §14 guard, the §20 clamp and its reported clamp reasons, the §21 FIFO slot and keyed blocker set, and the final live authorization fence apply unchanged, and a fixed duration above a configured cap is clamped exactly as the action's would be rather than refused or reinterpreted.
>
> Neither change touches the five controller states, T1-T59 (IDs, topology, guards, actions, destinations, reasons), I1-I37, runtime Store schema 3, the §23.4 verification contract, the actuator-hazard identity model, accounting, or the Home Assistant 2025.9.0 minimum. The normative behavioural inventory gains exactly one new ID, **MF6**, for the fixed-duration button admission path, so §39.2 becomes **142** IDs (MF1-MF6) instead of 141. Operators of devices that enforce their own outlet duration (for example a multi-outlet tap timer whose internal per-outlet timer closes the valve independently) must raise that device-side duration to match a longer pulse; a longer `pulse_duration` alone does not extend a duration the actuator enforces outside Home Assistant, and MoistureLoop continues to report such a takeover as `external_actuator_state_change` rather than concealing it.
>
...
Most irrigation controllers are open loop: they run a timer, calendar, weather, or evapotranspiration calculation and optionally use moisture as a veto. MoistureLoop instead treats measured soil moisture as the authoritative automatic feedback signal.
...
`async_track_state_change_event` does **not** receive unchanged reports. MoistureLoop therefore installs both:
...
**Final canonical identity: MoistureLoop, domain `moisture_loop`.** The project was developed under the working name Moisture Loop (`moisture_loop`), was renamed SoilSync on 2026-08-24 after a technical collision check, and on 2026-08-27, before any tagged release, returned to MoistureLoop because an unrelated agricultural/software business materially overlaps the SoilSync name. The 2026-08-27 re-check found no `moisture_loop` integration in the current Home Assistant Core component tree and no unrelated `embersas/moisture-loop` repository. Adjacent irrigation projects remain distinct. ([Core integration catalog](https://github.com/home-assistant/core/tree/dev/homeassistant/components), [HACS default repository catalog](https://github.com/hacs/default))
...
2. Add one MoistureLoop integration entry.
3. Add one or more zone subentries by selecting a name, sensor, actuator, thresholds, pulse/soak timing, and safety limits. The final flow step states that new zones are created disabled.
...
5. A newly created zone is disabled: its status is `DISABLED` and its enable switch is off. Creation performs no automatic or manual watering and issues no actuator ON, even when the moisture reading is already below the start threshold and every other gate would pass. The user reviews the zone and then explicitly turns the enable switch on. Enabling an already dry, fresh, otherwise eligible zone may start automatic watering immediately; that is the intended meaning of the explicit action.
6. Normal automatic operation runs quietly and reports session outcomes.
7. Manual watering is requested with an explicit duration through `moisture_loop.start_manual_watering`.
8. Sensor-only faults still permit bounded manual watering. Actuator, configuration, and integrity faults do not.
9. Reconfiguration may cooperatively prepare an active old session, records `CONFIG_CHANGED`, updates with `async_update_and_abort`, and is applied by the entry reconciler; at most one reload is scheduled when entity reconstruction requires it. If the actuator changes, the old actuator record is retained, the new actuator is resolved independently, and the logical zone's conservative budget/interval history continues.
10. Native zone deletion returns through Home Assistant's normal UI path. MoistureLoop immediately rejects new ON from the changed mapping, then completes safe closure in the background. A deleted-zone Repair may remain visible at entry level until its retained tombstone is safe and acknowledged.
...
| `pulse_duration` | duration | 60 min | 30 s-60 min; <= session and daily limits |
| `soak_duration` | duration | 20 min | 1 min-4 h |
| `max_cycles` | integer | 2 | 1-20 |
| `max_session_runtime` | duration | 2 h 5 min | pulse duration-4 h |
| `max_daily_runtime` | duration | 4 h | pulse duration-12 h |
...
| `manual_max_duration` | duration | 60 min | 1 min-2 h |
...
MoistureLoop listens to both changed states and unchanged reports as defined in §5.2. A repeated identical report is a real observation: it advances `last_reported`, refreshes the observation and an active AUTO freshness deadline, can auto-clear a stale fault, and can qualify as the post-soak report if its timestamp satisfies §18.4.
...
| Stage-1 shutdown vs `EVENT_HOMEASSISTANT_STOP` | no competition exists: the event owns no MoistureLoop safety work and fires only after Stage 1 returned or timed out |
...
Externally owned flow is persisted and respected, not counter-commanded merely because Home Assistant is stopping. A correctly persisted `external_flow` owner/blocker is successful MoistureLoop safety handling and does not by itself make the process unclean.
...
Core runs the registered shutdown jobs in Stage 1 of `HomeAssistant.async_stop()` and awaits them together under Core's Stage-1 timeout. Stage 1 executes strictly before Core cancels background tasks, before Core sets `CoreState.stopping`, and before Core fires `EVENT_HOMEASSISTANT_STOP`. The Stage-1 job is therefore the only MoistureLoop lifecycle hook that can both drive cooperative OFF and complete the mandatory §23.4 immediate fresh-Store verification.
...
`SHUTDOWN_OFF_BUDGET_S` is the maximum total time the authoritative Stage-1 MoistureLoop shutdown owner waits for cooperative active-flow closure, including convergence of an already in-flight ON and the shared OFF operation, before recording conservative unclean shutdown handling. It is one overall active-flow deadline. It is not a delay before OFF, not an allowance per controller, and not a universal Docker or process-manager setting; OFF signalling begins immediately. The external deployment process-stop window, for example a container stop grace period, remains distinct from Home Assistant's internal Stage-1 timeout, and MoistureLoop must fit inside both.
...
External flow that MoistureLoop does not own is persisted and respected. MoistureLoop does not counter-command an externally owned actuator merely because Home Assistant is stopping, and a correctly persisted `external_flow` owner/blocker is successful shutdown handling.
...
`EVENT_HOMEASSISTANT_STOP` has no MoistureLoop shutdown ownership. It does not dispatch `HOME_ASSISTANT_SHUTDOWN`, initiate or join OFF, persist safety state, fresh-read the Store, mark the run clean, or compete with the Stage-1 owner.
...
Creating a genuinely new logical zone is additionally governed by the following normative rules:

1. The first `zone_runtime` minted for that zone has `enabled = false` and state `DISABLED`. This is the single authoritative fresh-zone default; there is no other initialization layer, entity default, or flow step that may set it true.
2. That value is part of the same serialized, read-back-verified safety transaction that persists the new record, identity, applied shadow, and zone history. It is durable before any controller for the zone is constructed, before its listeners exist, before it is published to the entity platforms, and before admission can offer it a slot, so no interval exists in which a zone is watering-eligible merely because creation completed. Creation is never implemented as enable-then-disable, as an active zone followed by a stop, or as post-creation cleanup.
3. While `enabled` is false, no AUTO or MANUAL admission is possible: the `G-EN` guard fails, `G-MANUAL-SAFE` fails, no session is created, no slot is requested or granted, no possible-flow ownership or open accounting arises, and no actuator ON is issued. This holds unconditionally even when the sensor is VALID and FRESH, the moisture value is strictly below the start threshold, the actuator is available and proven OFF, no blocker exists, and both session and daily budgets permit watering. A dry, fresh, or newly arriving moisture report never implicitly enables the zone; each such report re-evaluates and stays `DISABLED`.
4. State `DISABLED` follows from `enabled = false` through the same `enabled`-first derivation used in rule 9 below and in §25.4/§25.5. It is reconciliation output, not a new transition row, and it adds no sixth controller state.
5. Only an explicit user enable through a supported control path (the §28.3 enable switch) lifts this. Enabling then behaves exactly as `T46`/`T47` already specify: the zone becomes `FAULT` if a fault applies, otherwise `IDLE` with an evaluation scheduled, and AUTO may start immediately if every §14 guard passes. No additional armed state, confirmation entity, or second enable step exists.
6. This rule applies to fresh creation only. Every adoption path preserves the persisted `zone_runtime.enabled` of an existing zone and must never overwrite it with the fresh-zone default: same-subentry continuity across restart and reload, non-actuator reconfiguration, exact durable-identity delete/re-add under §25.5, schema-1 migration under §23.2.1, and A -> B actuator replacement under rule 8 below. Durable runtime state is authoritative wherever it exists; an existing zone's `enabled` value is never re-derived from a configuration default.
...
The flow presents the removed actuator's last-known entity ID and the consequence in plain language, and requires an explicit, **unchecked-by-default** confirmation control asserting that the removed actuator is physically off and can no longer deliver water. It is a safety assertion, not a dismissal, and MUST NOT be presented as "dismiss" or "ignore". Home Assistant seeds a fix flow with its issue data and delivers that seed to the flow's first step as user input; a MoistureLoop acknowledgement step MUST therefore treat only a submission that actually carries the confirmation field as the operator's assertion, so opening a Repair can never acknowledge anything. The same rule applies to the §26.3 flow.
...
- `manual_pulse_15`, `manual_pulse_30`, `manual_pulse_60` buttons: one fixed-duration manual watering request each, of 15, 30, and 60 minutes.

The manual buttons exist because a bounded duration *can* be supplied by a button press when it is fixed in the control's own identity rather than prompted for; what a button cannot safely supply is an *open-ended* or operator-typed duration, and none of these do. Each press is an ordinary `async_manual_start` request: it is subject to every §14 guard, the §20 clamp against `manual_max_duration`, `max_session_runtime`, and remaining daily budget, the §21 FIFO slot, and the final live authorization fence, and it decides nothing itself (I27). A press whose fixed duration exceeds a configured cap is clamped and reported through the existing §20 clamp reasons, exactly as the action is; it is not refused and not silently reinterpreted. The open-ended manual start remains action-only (§31), because only an action can carry a caller-supplied duration. These device-targeted controls are only for current configured zones. A deleted-zone acknowledgement uses the exact-record Repair flow in §26.3.
...
- **MF6:** each §28.3 fixed-duration manual button requests exactly its own bounded duration through `async_manual_start`, is refused by the same §14/§20 guards as the action with the same translated reasons, is clamped rather than refused when its duration exceeds a configured cap, and never commands an unbounded ON.
...
- **LC14:** a genuinely new zone is created `enabled=false` in DISABLED by its one verified creation transaction, and its persisted runtime carries that value before any controller, listener, entity, or slot admission exists. With the sensor VALID and FRESH, moisture strictly below the start threshold, the actuator available and proven OFF, no blocker present, and both budgets permitting, creation and every subsequent qualifying report still produce no AUTO session, no MANUAL admission, no slot request or grant, no possible-flow owner, no open accounting, and no actuator ON; only the explicit enable switch lifts it, through the existing T46/T47 outcomes. Existing zones are unaffected: a persisted enabled zone stays enabled and a persisted disabled zone stays disabled across reload, restart, reconfiguration, exact-identity re-add, and schema migration, and reconciliation never overwrites a persisted `enabled` with the fresh-zone default.
...
- **TB15:** certification requires an explicit confirmation control that is never checked by default; opening the Repair is never itself the assertion, even though Home Assistant seeds a fix flow with its issue data. It refuses record/lineage mismatch and stale preconditions visibly at submit time, commands no actuator, starts no watering, changes no zone's settings or enabled state, releases only the certified lineage's keys, and leaves every other blocker effective.
...
| external ON outside a session | respect actor, keyed global resource blocker until proven OFF | MoistureLoop cannot bound external flow itself |
...
- `0.1.1` combines two independent changes. The setup-flow pass is presentation only: labels, field help, and step copy, with no persisted key, default, bound, guard, or transition change. The removed-actuator recovery of §26.4 is behavioural and advances the specification to `0.1.0-spec.7`; it is documented and reviewed separately from the presentation work. Released `0.1.1` ships both changes together, so its authoritative release scope is the combined one: specification `0.1.0-spec.7`, runtime Store schema 3, and a normative inventory of 141 behavioural IDs, I1-I37, and T1-T59.
...
- Domain/name are finalized as MoistureLoop / `moisture_loop` before the first release.
