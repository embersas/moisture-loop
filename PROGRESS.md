# MoistureLoop Implementation Progress
...
> **Development-name history:** The project was developed as **Moisture Loop** (domain/package `moisture_loop`, repository slug `moisture-loop`), was canonically renamed **SoilSync** (`soilsync`) on 2026-08-24, and on 2026-08-27, before any tagged release, returned to the canonical identity **MoistureLoop** (`moisture_loop`, `custom_components/moisture_loop/`, `embersas/moisture-loop`) because of an external name-collision concern. Occurrences of `Moisture Loop`, `SoilSync`, `soilsync`, old commands, old file lists, old URLs, and SoilSync-named validation evidence in the dated session logs below are retained only as truthful historical evidence of the era in which they were current. They are not current installation, domain, package, service, event, Store, CI, or repository instructions.
