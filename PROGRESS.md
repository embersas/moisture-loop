...
- Current spec.4 conformance: `Spec.4 Remediation Stages 1-3 are complete. Canonical schema-2 models/migration, exact-record blockers, immutable entry snapshots, serialized latest-generation reconciliation, config+Store startup union, canonical configured-record materialization, tombstones, exact same-record reactivation, and A -> B runtime orchestration are current. Slices remain conservatively [~] where Stage 4 final-ON fencing, Stage 5 flow/reload ownership, Stage 6 surfaces, Stage 7 full traceability, or Stage 8 release evidence is still outstanding. Slices 2 and 6 remain fully conformant within their documented scopes.`
...
- Next implementation work: `Stage 4 is technically unblocked by the completed Stage-3 runtime ownership interfaces, but may begin only after new explicit user authorization. No implementation work is currently authorized.`
- Release gates: `Spec.4 Remediation Stages 4-8 and the integrated 134-ID evidence remain outstanding; the supported-current Home Assistant harness has not been evidenced; GitHub-hosted hassfest and HACS Action have not been evidenced; all §46 prototype validations remain outstanding.`
- Slice 13: `Not started. No mock or existing automated result is treated as §46 prototype evidence.`
- Overall status: `Not release-ready. Stages 1-3 are complete. The Stage-3 focused HA 2025.9.0 coverage suite has 593 passed/1 deliberate pure-boundary skip and 91.67% combined affected-module branch coverage (slot_manager.py and state_machine.py 100%); the pure Python 3.14.5 suite has 423 passed. The post-Stage-3 broad inventory has 674 passed, 2 Stage-6-owned surface expectation failures, 1 deliberate skip, and 0 errors. The formerly classified teardown error is eliminated. Historical spec.3 and Stage-1/2 evidence remains preserved.`

On 2026-08-22 the user explicitly authorized and completed Spec.4 Remediation Stages 1, 2, and 3 in sequence. Stage 3 is complete and the authorization has returned to `None`. The 2026-08-21 instruction "implement as per progress.md" remains recorded only as the historical authorization under which the spec.3 implementation was produced. Any Stage 4-8 implementation or prototype work requires new explicit user authorization.
...
| 8 | Startup, reload, reconfiguration, and shutdown lifecycle | [~] Stage 3 complete; Stage 4-5 remediation remains |
...
- **Tests currently implemented:** Stage-1/2 evidence remains as recorded. Stage 3 adds runtime portions of PI24-PI27, LC13, ND1-ND3/ND6/ND13-ND16, TB5-TB11, AR1-AR17, and RC5-RC12 for listener ownership, immutable snapshots, canonical materialization, startup union/tombstones, exact-UUID reactivation/rename/conflict, A -> B hazard/history continuity, coalescing, failure, supersession, and lifecycle ownership. IDs whose complete proof depends on Stage 4 final-ON fencing, Stage 5 reload ownership, Stage 6 surfaces, or Stage 7 full traceability remain explicitly partial.
- **Tests actually run/passing:** the 2026-08-22 Stage-3 focused HA 2025.9.0 coverage command passed 593 tests with 1 deliberate pure-boundary skip and 91.67% combined affected-module branch coverage; `state_machine.py` and `slot_manager.py` remain 100% branch covered. The pure Python 3.14.5 suite passed 423 tests. The post-Stage-3 broad inventory produced 674 passed, 2 Stage-6 surface expectation failures, 1 deliberate skip, and 0 errors. The formerly classified service teardown error passes in isolation and is absent from the broad run. Historical 2026-08-21 and Stage-1/2 totals remain unchanged below.

## Spec.4 Remediation Assessment

This assessment compares the approved spec.4 requirements with the dated spec.3 baseline and current remediation state. Stages 1-3 now declare Store schema 2, persist canonical `safety_records` plus independent `zone_histories[*].zone_runtime`, key live/durable blockers by `safety_record_id`, fail grant admission closed for reconciliation state, provide verified conservative history handoff without hazard transfer, and consume those authorities through the entry-wide configuration coordinator and startup config+Store union. `ZoneRecord` remains only a non-serialized compatibility projection at the exact-record controller persistence seam. Config flow still owns the old reload behaviour, the complete Stage-4 final-ON/in-flight fence is absent, and Repairs/diagnostics/entities still require Stage-6 full schema-2 surface remediation. Those remaining observations assign Stages 4-8 and are not Stage-1/2/3 defects.
...
| 4 | `[~] Stages 1-3 complete; later integration evidence required` | Schema 2/migration, exact-record writes, startup union, configured materialization, tombstones, reactivation, and verified A -> B history handoff are implemented. Stage 4 controller dispatch persistence and Stage 7 complete evidence remain. |
...
| 7 | `[~] Stage 3 ownership complete; Stage 4 required` | Controllers now require exact canonical records, persist through exact-record APIs, carry applied generation/lifecycle metadata, and become observation-only when retained. The complete final pre-ON membership/fingerprint/generation gate, no-suspension dispatch boundary, in-flight marker, post-call recheck, and compensation remain Stage 4. |
| 8 | `[~] Stage 3 complete; Stages 4-5 required` | EntryRuntime now owns the supported listener/coordinator, immutable snapshots/generations, config+Store union, lifecycle materialization, exact reactivation, A -> B orchestration, and stale/failure/reload/shutdown fencing. Stage 4 final-ON races and Stage 5 reconciler-owned reload application remain. |
| 9 | `[~] Stage 3 listener integration complete; Stage 5 required` | Native public add/update/removal mutations feed the entry listener and tombstone reconciliation. Config flow still has add-owned reload scheduling and `async_update_reload_and_abort`; Stage 5 must replace those without creating a competing owner and Stage 7 must run the actual websocket route. |
...
Stages 1-3 were explicitly authorized in sequence and are complete. No later stage is authorized or started. A later stage may be marked complete only after its named evidence is implemented and actually run.
